...
const ScanResults = () => {
 ...
 const alternativeRecommendations = [
        { name: "Almond Milk", imageUrl: "https://media.bluediamond.com/uploads/2022/11/03104320/almond-breeze-almondmilk-unsweetened-original.png" },
        { name: "Soy Milk", imageUrl: "https://soy.com.au/wp-content/uploads/2017/08/UHT-Soy-Milk-Original-3D.png" },
        { name: "Oat Milk", imageUrl: "http://t0.gstatic.com/images?q=tbn:ANd9GcTHcSdj9ghZO4UEd4OLnRqpusM5AVn17XGZkI6heSnluLl52OoTHAUOitYbt8gAhPbfNvF0KA" }
...
const navigateToRecipes = () => {
    ...
    const product = {
        ...
        reviews: [
            ...
            { user: "User2", rating: 3, comment: "Good quality but a bit expensive." }
        ]
    };

    return (
        <View style={{height:800,padding:15}}>
            ...
            <View style={styles.box}>
                ...
                <Text style={styles.text}>Calories: {product.nutritionalInfo.calories}</Text>
                <Text style={styles.text}>Carbohydrates: {product.nutritionalInfo.carbohydrates}g</Text>
                <Text style={styles.text}>Proteins: {product.nutritionalInfo.proteins}g</Text>
                <Text style={styles.text}>Fats: {product.nutritionalInfo.fats}g</Text>
                <Text style={styles.text}>Vitamins: {product.nutritionalInfo.vitamins.join(', ')}</Text>
                <Text style={styles.text}>Minerals: {product.nutritionalInfo.minerals.join(', ')}</Text>
            </View>
            <View style={styles.box}>
                <Text style={styles.sectionHeading}>Product Reviews and Ratings</Text>
                {product.reviews.map((review, index) => (
                    <View key={index} style={styles.commentBox}>
                        <Text style={styles.review}><Text style={styles.bold}>{review.user}</Text> - Rating: {review.rating}</Text>
                        <Text style={styles.comment}>{review.comment}</Text>
                    </View>
                ))}
            </View>
             <View style={styles.box}>
                                <Text style={styles.sectionHeading}>Alternative Recommendations for Lactose Intolerant</Text>
                             
                    {alternativeRecommendations.map((item, index) => (
                           <View key={index} style={styles.alternativeBox}>
                                  <View style={{display:"flex", flexDirection:"row", gap:15}}>
                            <Text style={styles.text}>{item.name}</Text>
                            <Image source={{ uri: item.imageUrl }} style={{height:50, width:50}} />
</View>
                        </View>
                    ))}
                </View>
